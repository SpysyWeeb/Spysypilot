...
which in order: builds the lead0/lead1/stop obstacle columns; finds the binding column
(`argmin(x_obstacles[0])`) and, whenever it changes (lead0/lead1/stop, any direction), refills `a_prev` with the current
acceleration (D14); applies `jerk_scale`/`t_follow_pad` only while lead0 is the source; calls
...
pinning the target at the kiss the two agree. Ruling (audit 2026-09-02): every candidate is subject to the corridor, the curve
...
turn budget (D2). `update()` order: reset state → lead and anchors → supervisor → stop observation →
