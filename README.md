...
- ⚠️ **[Better lateral tune v2 (BLaTv2)](https://github.com/SpysyWeeb/Spysypilot/tree/BLaTv2)** — v220 uses one scalar-faithful inverse-EPS torque path: it replays the real queued actuator delay, solves the model-authored terminal angle and rack speed, projects only the authored inverse feedforward through exact 409/4/7 reachability, and applies measured-rack feedback once; four-route raw, applied, and delivered replay beat v14 without preview boosts, filters, persistence gates, or low-speed mode switches, while field validation remains in progress; passive frozen-v14 shadow v16 provides the live A/B &nbsp;*(personal idea)*
