...
- ⚠️ **[Better lateral tune (BLaT)](https://github.com/SpysyWeeb/Spysypilot/tree/BLaT)** — restored to the field-tested controller-v14 future-path and actuator-aware Hyundai torque-control baseline with (409, 4, 7) command/safety limits and EPS-motion-gated damping; the v15–v19 interventions remain removed, and the replacement architecture has emitted its Phase 0 stock/V14 route-replay baseline in route-audit commit `acdc165` before any v15 controller work; replay rows now use controller+opendbc commit identity, and every future behavior change must bump the logged controller version in the same commit &nbsp;*(personal idea)*
