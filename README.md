...
- ⚠️ **[Always-On-Lateral (AOL)](https://github.com/SpysyWeeb/Spysypilot/tree/AOL)** — steering is toggled separately from cruise control, so the driver can choose to use op long without enabling op lateral; runs its own state machine beside selfdrived's, with real panda/opendbc safety-layer support via the SpysyWeeb submodule forks &nbsp;*(personal idea)*
