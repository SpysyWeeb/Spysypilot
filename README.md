...
- ⚠️ **[Better lateral tune v2 shadow foundation (BLaTv2)](https://github.com/SpysyWeeb/Spysypilot/tree/BLaTv2)** — telemetry-only plant/reference shadow now runs in `combo` to collect residual and runtime data while the field-tested v14 controller remains solely responsible for steering; shadow v5 builds one shared candidate workspace, hard-bounds MPC to one deterministically warm-selected active-set solve, and runs below `controlsd` on the same real-time core so device timing measures the candidate instead of ordinary scheduler stalls; manager restarts this passive process after a crash; no BLaTv2 actuation path exists &nbsp;*(personal idea)*
