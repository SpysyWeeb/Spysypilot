...
- ⚠️ **[Better lateral tune v2 shadow foundation (BLaTv2)](https://github.com/SpysyWeeb/Spysypilot/tree/BLaTv2)** — telemetry-only plant/reference shadow now runs in `combo` to collect residual and runtime data while the field-tested v14 controller remains solely responsible for steering; shadow v6 builds one shared candidate workspace, evaluates one active-set schedule, and selects it in O(1) by reusing the prior winning ordinal; CPU 4 affinity is temporarily retained to measure selector and scheduler effects separately; manager restarts this passive process after a crash; no BLaTv2 actuation path exists &nbsp;*(personal idea)*
