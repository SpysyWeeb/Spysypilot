...
- ⚠️ **[Sometimes-On-Lateral (SOL)](https://github.com/SpysyWeeb/Spysypilot/tree/AOL)** — steering is toggled separately from cruise control, so the driver can use op lateral without enabling op long, or op long without enabling op lateral; runs its own state machine beside selfdrived's, with real panda/opendbc safety-layer support via the SpysyWeeb submodule forks; formerly Always-On-Lateral (AOL), which the branch and code identifiers still use &nbsp;*(personal idea)*
...
- ⚠️ **[Better longitudinal tune (BLoT)](https://github.com/SpysyWeeb/Spysypilot/tree/BLoT)** — make braking and acceleration feel like my own driving: a necessity supervisor that drives the MPC's own runtime knobs instead of wrapping its output, a forecast trust ledger that plans against the driving model's predicted lead trajectory (comma PR [#37824](https://github.com/commaai/openpilot/pull/37824)) with radar as auditor rather than gatekeeper, and a proportional launch chain; see [docs/BLoT.md](docs/BLoT.md); step 1 was [op-model-grader](https://github.com/SpysyWeeb/op-model-grader), which grades the model's driving from rlogs against my own manual driving &nbsp;*(personal idea)*
- ⚠️ **[Better lateral tune (BLaT)](https://github.com/SpysyWeeb/Spysypilot/tree/BLaT)** — more steering authority for tight curves and 90° turns: Hyundai torque limits (409, 4, 7) vs stock (384, 3, 7), changed in both the command layer and the panda safety layer via the [SpysyWeeb/opendbc](https://github.com/SpysyWeeb/opendbc) fork &nbsp;*(personal idea)*
