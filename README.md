...
- ⚠️ **[Better lateral tune v2 (BLaTv2)](https://github.com/SpysyWeeb/Spysypilot/tree/BLaTv2)** — the tournament-selected inverse-EPS LQI controller v202 uses a six-node live-field-calibrated steady steering-load schedule and 0.09 breakaway friction with exact measured-to-desired rack-rate zero-crossing averaging instead of hard Coulomb sign flips; the byte-identical frozen-v14 pipeline runs passively in shadow v8 for live A/B evidence, while invalid LQI output uses the existing `commIssue` soft-disable/no-entry path after its bounded hold-and-decay sequence; promotion remains in progress pending recalibrated replay gates, device timing, and owner field testing &nbsp;*(personal idea)*
