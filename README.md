...
- ⚠️ **[Better lateral tune v2 (BLaTv2)](https://github.com/SpysyWeeb/Spysypilot/tree/BLaTv2)** — the tournament-selected inverse-EPS LQI controller v202 uses a six-node live-field-calibrated steady steering-load schedule and 0.09 breakaway friction with decision-cell zero-crossing averaging instead of hard Coulomb sign flips; passive shadow v8 retains low-control real-time priority but roams cores 0–4 after CPU-4 pinning reduced it to ~32 Hz, and its v14 stream remains diagnostic-only until near-100 Hz throughput restores replay consistency; invalid LQI output uses the existing `commIssue` soft-disable/no-entry path after its bounded hold-and-decay sequence; promotion remains in progress pending response-surface replay, device timing, and owner field testing &nbsp;*(personal idea)*
