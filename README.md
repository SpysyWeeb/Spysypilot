...
- ⚠️ **[Automatic longitudinal event logger](https://github.com/SpysyWeeb/Spysypilot/tree/auto-long-logger)** — detects late lead-launch timing on-device, attributes delay to planner/controller/vehicle response, shows **Long Event Logged**, preserves surrounding segments, and writes an SSH-readable manifest; the bookmark button also records a manual event; see [docs/AutoLongLogger.md](docs/AutoLongLogger.md) &nbsp;*(personal idea)*
