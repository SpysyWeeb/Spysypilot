...
(onset unchanged) because its 5 °/s wheel-rate gate sits above the 2–5 °/s of such moves; a speed-scaled gate is the follow-up. The preview-flicker units fix (2819d9879: the scheduler's frame-to-frame flicker tolerance was a fixed 0.25° of wheel against a re-plan noise that is a constant lateral acceleration, so it fired on two thirds of frames below 8 m/s) merged 2026-09-08 and ✅ field-validated 2026-09-10 (route 00000062, 46 engaged min: the gate fires on 5.3 / 1.8 / 0.6 % of frames below 8, 8–20 and above 20 m/s against 67.6 / 35.4 / 15.0 for the old formula; deep preview collapses above 8 m/s −30–45 % against the five pre-fix routes; served target and torque inside the pre-fix range at every speed; owner: no difference noticed). The rate-source fix (FM3.8 as written: wheel rate from an alpha-beta tracker on the 0.1° angle, the 4 °/s unsigned SAS_Speed kept only as validity) merged 2026-09-10 (bbc04e496) and ✅ field-validated the same day on routes 00000063/64 (23 engaged min, town and rural, nothing above 82 km/h: the estimate matches the wheel's rate 0.9–1.0× with the sign right 91–96 % above 4 °/s, the D term now opposes the wheel's motion, no invalidations; torque content at 2–5 Hz up as replay predicted and not felt; owner: no complaints). Highway measured 2026-09-11 on route 00000069 (54 engaged min, 30 at 86–130 km/h): near:wheel at 0.55–0.8 Hz 1.46 ± 0.03 at 112–144 km/h (n = 77) against 1.59 pre-fix and 1.71 on the same car two days earlier, the plan stage unchanged at 1.25 and the reduction in the wheel stage where the D term now damps; costs: per-frame torque change p99 0.039 → 0.047 and 2–10 Hz torque content +36–66 % at 31–40 m/s, the D term reacting to the wheel's own high-frequency motion. The remaining 1.25 at the plan stage was the resonance fix's target (route-audit `phase3/ablation_2026-09-11/ABLATION.md`, `phase3/resonance_fix_2026-09-11/DESIGN.md`). The resonance fix (95bde9d99, 635921d9f, 5db5130b4, a86489017: one tracker law `a = wn²(r − x) + 2wn(k·ṙ − ẋ)` with k = 0.925 and a 0.3 s response time, the served rate as the served position's own motion in every branch, the horizon fit removed, a held frame as elapsed time, the intended angle as the model's target; two adversarial reviews) merged 2026-09-11 (combo 6fe13d833) and ✅ field-validated the same day on route 0000006a (47 engaged min, town to 136 km/h: near:planned at 0.55–0.8 Hz 0.92 / 0.95 / 1.01 at 112–144 / 86–112 / 61–86 km/h against 1.25 / 1.08 / 1.00 before and the replay's 0.95 / 0.91 / 0.94, every well-sampled reference cell within 0.05 of replay; near:wheel at 112–144 km/h 1.20 ± 0.07 against 1.46 on the rate-source build, 1.59 pre-fix and stock's 0.88; resonance-band torque −37–45 % at 31–40 m/s, 1.2–2 Hz +26 %; below 10 m/s plan overshoot 12 → 3 %, guard duty 4.9 → 2.1 %, steps and wrong-way duty down, no invalidations; owner: "drove better"). What remains of the highway amplification is the wheel stage (planned:wheel 1.13–1.30), the torque plant; the hold top-up is now the largest torque-side term at speed (route-audit `phase3/field_2026-09-11_resonance_6a/REPORT.md`). The hygiene batch (0b6580bf6, 8d30ed981, 988698c6b, 0d5147f5a: the frame's inline torque rules named and extracted with per-rule constants, bit-identical on every replayed frame; a content fault holds the plan and the wheel instead of resetting and handing over; the rack/stock hand-over slewed at the R7 step; modeld's preview grid pinned to the controller's; the held torque through the driver-assist envelope; two adversarial reviews) merged 2026-09-11 (combo b3560d780) and ✅ regression-checked the same day on routes 0000006b/6e (21 engaged min, town and rural to 82 km/h): both drives replayed through the batch and the pre-batch controller are byte-identical on every output, no fault, hold or hand-over frame occurred (status 1 on every engaged frame, 0 fallbacks), ordinary metrics inside route-to-route spread; the new paths have run in tests and the hand-over harness only (route-audit `phase3/hygiene_batch_2026-09-11/`, `phase3/field_2026-09-11_hygiene_6b_6e/REPORT.md`). Step 2, the hold top-up in two regimes (1eeb6fc14: today's hold-effort term at and below 10 m/s, integral action in P's own units from 15 m/s up with K_i = K_p / 5 s, a 30 s leak and a 0.1 cap, blended between; the term had been measured as the loop's integrator in disguise, larger than P at highway speed with 60–85° of lag in the resonance band; reviewed) merged 2026-09-11 (combo 4042f55ca) and ✅ field-validated the same day on route 0000006f (52 engaged min, town to 129 km/h): with the road held fixed (the route's own frames through the previous and the new controller) the top-up's share of P in the 0.35–0.8 Hz band at 24–40 m/s fell 0.68 → 0.14, its rms at speed 0.032–0.044 → 0.008–0.011, in-band torque per plan −5–7 %, torque steps unchanged to 2 %, holds at or below 10 m/s bit-identical (18,428 frames), wrong-way duty +0.06–0.14 points at 17–31 m/s; the field's ratios equal the replay's; near:wheel at 112–144 km/h unchanged at 1.20 against the stock reference 0.88 — the wheel stage, phase 4; owner: "looks good" (route-audit `phase3/topup_2026-09-11/`, `phase3/field_2026-09-11_topup_6f/REPORT.md`). Step 3, one preview scheduler with a continuous served preview (899e4a0ad, 6bdcf3238: the model-confidence gate, a driver-disengage predictor that kept the preview closed on 70–94 % of frames below 24 m/s, is gone; the 40 m cap, a speed clock that kept it under 1.25 s above 24 m/s, is gone in favour of the 2 s horizon; the heading gate is in lateral acceleration (0.2 m/s²) instead of wheel degrees; the envelope drift gate and the second scheduler are gone; the served preview eases at 0.2 s per frame instead of stepping, so the reference filter's time constant and the tracker's response time never jump; reviewed) merged 2026-09-11 (combo d60285d93) and ✅ field-validated 2026-09-12 on route 00000070 (47 engaged min, town to 129 km/h): the served preview averages 1.5–1.7 s at every speed against 0.03–0.55 before, at zero on 3–14 % of frames against 44–98 %, largest per-frame change exactly its 0.2 s bound; near:wheel at 0.55–0.8 Hz 0.89 at 112–144 km/h against 1.20 before and the stock reference 0.88 (0.95 / 0.90 at 86–112 / 61–86 against 1.13 / 1.15; phase 28° behind stock), 0.61 at 0.8–1.2 Hz against 0.94 (stock 0.79); with the road held fixed, commanded torque content −12–30 % in every band from 0.2 to 3.5 Hz at 24–40 m/s; low-speed steps and guard duty down; turn-in onset (route-4d gauntlet, replay) median 0.0 ms, p90 54 ms, 13 of 114 legs over 40 ms later — the calm filter's price; one 0.5 s stale-model event at 29 m/s handled by the hand-over path with a largest torque step of 0.038; owner: "drive felt fine, I think I did notice the lag change". What remains against stock is the 0.35–0.55 Hz band (1.06–1.08 against 0.91–0.94), the wheel stage, phase 4 (route-audit `phase3/preview_2026-09-11/`, `phase3/field_2026-09-12_preview_70/REPORT.md`). **❌ Reverted 2026-09-12 (d3ed2b3f4)** after routes 00000071/72 (town and rural, 20 engaged min): owner: "it feels lazy, and feels like it wanders in the lane too much"; the logs agree — the calm filter and the longer response time were open on bends (the scheduler's gates test frame-to-frame consistency, which a consistently drawn curve passes, not the target's steadiness; the preview was at full calm on 48–66 % of bend frames at 10–24 km/h against 0 % before), median lane-centre offset from the model's lane lines +25–40 % against the previous two drives, the chain at 61–86 km/h below stock's gain with 8–34° more lag. The highway measurement on route 00000070 stands. The controller is back to the state field-validated on route 0000006f. The redesign separates the two questions the scheduler fused: the preview keeps judging the path's consistency (R4's envelope opening) and the calm is earned by the admitted horizon's steadiness, continuously; it returns with replay evidence on routes 6f/70/71/72 first (route-audit `phase3/field_2026-09-12_wander/`).**
