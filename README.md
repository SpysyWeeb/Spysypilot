...
- ⚠️ **[Better lateral tune v2 (BLaTv2)](https://github.com/SpysyWeeb/Spysypilot/tree/BLaTv2)** — the tournament-selected inverse-EPS LQI is integrated as the Hyundai torque controller for acceptance testing; the byte-identical frozen-v14 pipeline now runs passively in shadow v7 for live A/B evidence, while invalid LQI output uses the existing `commIssue` soft-disable/no-entry path after its bounded hold-and-decay sequence; promotion remains in progress pending replay gates, suites, device timing, and owner field testing &nbsp;*(personal idea)*
