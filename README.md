...
- ⚠️ **[Better lateral tune v2 shadow foundation (BLaTv2)](https://github.com/SpysyWeeb/Spysypilot/tree/BLaTv2)** — telemetry-only plant/reference shadow now runs in `combo` to collect residual and runtime data while the field-tested v14 controller remains solely responsible for steering; its publisher normalizes solver values at the Cap'n Proto boundary and manager restarts this passive process after a crash; no BLaTv2 actuation path exists &nbsp;*(personal idea)*
