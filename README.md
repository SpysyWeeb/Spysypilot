...
- ⚠️ **[Better lateral tune v2 (BLaTv2)](https://github.com/SpysyWeeb/Spysypilot/tree/BLaTv2)** — v206 replaces the v203 LQI (**Linear–Quadratic control with Integral action**) cost trade with a delay-compensated inverse-rack action controller and decouples its fixed action timestamp from model smoothing: the model scalar remains the position authority, physical rack dynamics request full torque when needed, and Hyundai's exact 409/4/7 envelope provides command smoothing; newest-route replay shows high authority in sharp low-speed turns, while the documented frozen-route timing/roughness deviations remain open until the first v206 field drive; passive frozen-v14 shadow v9 provides the live A/B &nbsp;*(personal idea)*
