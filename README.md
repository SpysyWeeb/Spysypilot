...
- ⚠️ **[Better lateral tune (BLaT)](https://github.com/SpysyWeeb/Spysypilot/tree/BLaT)** — more steering authority for tight curves and 90° turns: Hyundai torque limits (409, 4, 7) vs stock (384, 3, 7), changed in both the command layer and the panda safety layer via the [SpysyWeeb/opendbc](https://github.com/SpysyWeeb/opendbc) fork; plus split-band P in the torque controller — the boosted low-speed gain only acts on errors that persist, killing the stair-step torque punches from 20 Hz re-plan strides (bit-identical to stock ≥15 m/s) &nbsp;*(personal idea)*
