...
# MADS-specific event types (we reuse ET namespace for simplicity)
# lkasEnable  -> ET.ENABLE equivalent
# lkasDisable -> ET.USER_DISABLE equivalent
# silentLkasEnable -> ET.ENABLE with silent transition (for pause recovery)
  ...
  def update(self) -> tuple[bool, bool]:
    ...
    if self.has('immediateDisable') or self.has('userDisable'):
      if self.state != State.disabled:
        if self.has('silentDisable') or self.has('userDisable'):
          self.state = State.disabled
        else:
          self.state = State.disabled
      ...
      if self.has('lkasEnable') or self.has('silentLkasEnable'):
        if self.has('noEntry'):
          # Can enter paused if a recoverable condition blocks full enable
