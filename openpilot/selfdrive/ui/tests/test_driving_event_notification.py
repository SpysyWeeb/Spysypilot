...
class TestDrivingEventNotification(OpenpilotTestCase):
  def test_success_requires_marker_and_current_preservation(self):
  ...
  def test_domain_titles_are_owned_by_shared_renderer(self):
  ...
  def test_group_is_coalesced_without_hiding_failure(self):
  ...
  def test_any_safety_alert_delays_then_releases_notice(self):
    queue = EventNotificationQueue()
    queue.push(ack(), 1.0)
  ...
  def test_corrected_acknowledgment_replaces_failure_for_same_event(self):
    queue = EventNotificationQueue()
