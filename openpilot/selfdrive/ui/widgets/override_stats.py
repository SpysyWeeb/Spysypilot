...
class OverrideStatsWidget(StatsPageWidget):
    """General overview of how much of your driving is spent overriding the model's
    steering, and a coarse breakdown of that override time by driving context (turn /
    straight line / curve / lane change). See the dedicated Turn / Curve / Straight Line
    pages (next in the cycle) for detail on each. The breakdown is inferred from
    measurable conditions at override time, not the driver's actual intent - it's a
    "under what conditions" view, not a literal "why"."""
    ...
    def _draw_context_breakdown(self, x: int, y: int, w: int) -> int:
        ...
        rl.draw_text_ex(fn, "Buckets reflect driving context at override time, not confirmed intent",
                         rl.Vector2(x, y + 150), 26, 0, DIM)
        rl.draw_text_ex(fn, "See the Turn / Curve / Straight Line pages for detail",
                         rl.Vector2(x, y + 180), 26, 0, DIM)
        return y + 220
