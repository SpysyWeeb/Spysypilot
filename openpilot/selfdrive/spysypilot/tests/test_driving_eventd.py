...
class TestDrivingEventPlatform(OpenpilotTestCase):
  def test_unique_ids_and_group_correlation(self):
  ...
  def test_semantic_episode_group_survives_generic_time_window(self):
  ...
  def test_distinct_keys_ten_seconds_apart_get_different_groups(self):
  ...
  def test_group_key_last_seen_refresh_is_monotone_under_out_of_order_occurrence(self):
  ...
  def test_brand_new_key_far_in_the_past_mints_a_new_group(self):
  ...
  def test_keyless_candidate_far_in_the_past_mints_a_new_group(self):
  ...
  def test_group_hard_lifetime_splits_a_pathological_same_key_chain(self):
  ...
  def test_retrying_an_accepted_event_retains_ids(self):
  ...
  def test_manual_lateral_and_longitudinal_coexist(self):
  ...
  def test_detector_exception_isolation(self):
    errors = []
    platform = DrivingEventPlatform(
  ...
  def test_lead_launch_and_stop_jolt_failure_isolation(self):
    errors = []
  ...
  def test_rolling_lead_and_standstill_launch_failure_isolation(self):
    errors = []
    platform = DrivingEventPlatform(
  ...
  def test_manual_input_does_not_suppress_or_replace_detector_output(self):
  ...
  def test_detectors_run_only_for_their_updated_sample(self):
  ...
  def test_submitter_retries_stable_id_until_acknowledged(self):
    class FakePubMaster:
      def __init__(self):
        self.messages = []
  ...
  def test_submit_failure_remains_pending_for_retry(self):
  ...
  def test_generic_domain_message_has_typed_empty_payload(self):
  ...
  def test_stop_jolt_schema_round_trip_preserves_peak_and_detection_times(self):
    ...
    _assert_approx_equal(self, payload.imuJerk, -3.5)
    _assert_approx_equal(self, payload.aEgoJerk, -3.0)
  ...
  def test_rolling_lead_schema_preserves_commit_and_later_detection(self):
  ...
  def test_lateral_candidate_uses_physical_crossing_and_later_confirmation_times(self):
  ...
  def test_road_confidence_penalty_applies_to_non_rescued_substantial_road_events(self):
    evidence = LateralEvidence(
      1.0, 2.0, 0.0, 0.0, False, 0.0, 0, 0.5, "lat:episode", 2.0, 2.0,
    ...
    _assert_approx_equal(self, candidate.confidence, max(0.90 - ROAD_SUBSTANTIAL_CONFIDENCE_PENALTY, ROAD_CONFIDENCE_FLOOR))
    _assert_approx_equal(self, candidate.confidence, 0.85)

  def test_road_confidence_penalty_never_applies_to_intervention_backed_or_assisted_unwind(self):
  ...
  def test_road_confidence_penalty_not_applied_when_road_interaction_not_substantial(self):
    evidence = LateralEvidence(
      1.0, 2.0, 0.0, 0.0, False, 0.0, 0, 0.5, "lat:episode", 2.0, 2.0,
    )
    detection = LateralDetection(
  ...
  def test_road_confidence_floor_is_an_invariant_not_a_reachable_branch(self):
    ...
    for confidence_in in (0.80, 0.90, 0.95, 1.0):
      ...
      candidate = lateral_candidate(LateralSample(1.0), detection)
      _assert_approx_equal(self, candidate.confidence, max(confidence_in - ROAD_SUBSTANTIAL_CONFIDENCE_PENALTY, ROAD_CONFIDENCE_FLOOR))
      assert candidate.confidence > ROAD_CONFIDENCE_FLOOR

  def test_lateral_build_message_wires_v6_present_bit_and_causation_fields(self):
    ...
    _assert_approx_equal(self, payload.highAngleUnwindScale, 0.42)
    _assert_approx_equal(self, payload.torqueCommandBeforeHighAngleExit, 0.33)
    _assert_approx_equal(self, payload.highAngleUnwindOldTorqueCorrection, 0.11)
    _assert_approx_equal(self, payload.highAngleUnwindOldDirectionTorque, 0.09)
    _assert_approx_equal(self, payload.oldTurnSign, -1.0)
    assert payload.driverCausation == "interventionBacked"
    assert payload.driverAssistRawTorqueOnly
    _assert_approx_equal(self, payload.unwindNeutralTorque, 0.18)
    _assert_approx_equal(self, payload.unwindPhaseDirection, -1.0)

  def test_lateral_build_message_high_angle_scalars_zero_when_invalid(self):
    evidence = LateralEvidence(
      1.0, 2.0, 0.0, 0.0, False, 0.0, 0, 0.5, "lat:episode", 2.0, 2.0,
  ...
  def test_lateral_names_times_windows_and_missing_optional_evidence_are_safe(self, internal_name, public_name):
    evidence = LateralEvidence(
  ...
  def test_live_pose_sampler_uses_its_own_timestamp_and_updates_bump_once(self):
  ...
  def test_longitudinal_sampler_uses_radar_present_field(self):
    class FakeSubMaster:
      def __init__(self):
        self.data = {
  ...
  def test_rolling_sampler_requires_a_radar_matched_valid_track(self):
    class FakeSubMaster:
      def __init__(self):
        self.data = {
          "radarState": SimpleNamespace(leadOne=SimpleNamespace(
  ...
  def test_lateral_sampler_uses_actual_caroutput_damping_not_torque_d_term(self):
    ...
    _assert_approx_equal(self, sample.actual_damping_amount, 0.031)
    assert sample.actual_damping_state == "damping"
    _assert_approx_equal(self, sample.breakaway_latch, 0.895)
    _assert_approx_equal(self, sample.sustain_floor_contribution, 0.805)
    assert sample.damping_version == 2
    _assert_approx_equal(self, sample.vertical_accel_deviation, 2.1)
    _assert_approx_equal(self, sample.measurement_rate, -0.22)
  ...
  def test_lateral_sampler_wires_crown_neutral_and_high_angle_fields_together(self):
    torque_state = SimpleNamespace(
      active=True, desiredLateralAccel=0.2, actualLateralAccel=0.1, p=0.3, d=-0.72,
      version=4, referenceVersion=4, referenceRate=0.5, referenceReachableTargetTorque=0.1,
      trackingMeasurementRate=-0.22,
    ...
    _assert_approx_equal(self, sample.unwind_neutral_torque, 0.18)
    _assert_approx_equal(self, sample.unwind_phase_direction, -1.0)
    _assert_approx_equal(self, sample.high_angle_unwind_scale, 0.42)
    _assert_approx_equal(self, sample.torque_command_before_high_angle_exit, 0.33)
    _assert_approx_equal(self, sample.high_angle_unwind_old_torque_correction, 0.11)
    _assert_approx_equal(self, sample.high_angle_unwind_old_direction_torque, 0.09)

  def test_lateral_sampler_high_angle_fields_all_none_together_when_schema_lacks_them(self):
    ...
    sample = lateral_sample(FakeSubMaster(), SteeringRateFilter(), False)
    _assert_approx_equal(self, sample.unwind_neutral_torque, 0.05)
    _assert_approx_equal(self, sample.unwind_phase_direction, 1.0)
  ...
  def test_lateral_sampler_defaults_crown_and_high_angle_fields_without_torque_state(self):
    class FakeSubMaster:
      def __init__(self):
        self.data = {
          "controlsState": SimpleNamespace(
