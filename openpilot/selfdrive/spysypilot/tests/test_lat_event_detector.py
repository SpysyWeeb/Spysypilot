...
def _assert_not_approx_equal(test_case: OpenpilotTestCase, actual: float, expected: float) -> None:
  tolerance = max(abs(expected) * 1e-6, 1e-12)
  test_case.assertNotAlmostEqual(actual, expected, delta=tolerance)
...
class TestLatEventDetector(OpenpilotTestCase):
  def test_lateral_detector_version_six(self):
  ...
  def test_stall_release_cycles(self):
  ...
  def test_tracking_inactive_cannot_release_a_stale_stall(self, inactive_s, inactive_samples):
    detector = LateralEventDetector(cooldown=0.0)
    now = 0.0
    for _ in range(20):
  ...
  def test_short_rate_transition_hysteresis_retains_real_release(self):
    detector = LateralEventDetector(cooldown=0.0)
    now = 0.0
    for _ in range(20):
      detector.update(sample(now, steering_rate_deg=0.0))
  ...
  def test_fast_center_crossing_without_reversal_is_center_overshoot(self):
  ...
  def test_fast_center_crossing_after_committed_reversal_is_harsh_handoff(self):
    event = fast_crossing(
  ...
  def test_desired_sign_blip_is_not_a_committed_reversal(self):
    detector = LateralEventDetector(cooldown=0.0)
    common = {
  ...
  def test_one_meaningful_reversal_sample_followed_by_zero_is_not_committed(self):
    detector = LateralEventDetector(cooldown=0.0)
    common = {
      "steering_rate_deg": -150.0,
      "unwind_effective_phase": 0.8,
  ...
  def test_normal_smooth_committed_reversal_is_silent(self):
    event = fast_crossing(
      LateralEventDetector(cooldown=0.0),
      desired_before=0.5,
      desired_after=-0.5,
  ...
  def test_brief_torque_gap_does_not_make_smooth_committed_reversal_harsh(self):
    detector = LateralEventDetector(cooldown=0.0)
    detector.update(sample(
  ...
  def test_tracking_error_persisting_before_crossing_counts_as_harshness(self):
    detector = LateralEventDetector(cooldown=0.0)
  ...
  def test_handoff_and_crossing_within_half_second_are_one_event(self):
    detector = LateralEventDetector(cooldown=0.0)
    detector.update(sample(
      -0.2,
      steering_angle_deg=30.0,
    ...
    _assert_approx_equal(self, events[0].evidence.phase_handoff_mono_time, 0.05)

  def test_committed_reversal_does_not_assume_wheel_and_trajectory_sign_mapping(self):
    ...
    _assert_approx_equal(self, event.evidence.handoff_center_delta_s, 0.25)
    assert event.evidence.signed_center_rate_deg == 315.0
    _assert_approx_equal(self, event.evidence.desired_lateral_accel_before_crossing, 0.236)
    _assert_approx_equal(self, event.evidence.desired_lateral_accel_after_crossing, -0.20)

  def test_handoff_and_unrelated_later_crossing_are_separate_events(self):
    detector = LateralEventDetector(cooldown=0.0)
  ...
  def test_crossing_occurrence_is_interpolated_and_detection_is_later(self):
    event = fast_crossing(LateralEventDetector(cooldown=0.0))
    assert event is not None
    _assert_approx_equal(self, event.occurred_mono_time, 0.275)
    assert event.detected_mono_time is not None
    assert event.detected_mono_time >= event.occurred_mono_time + 0.5

  def test_exact_zero_sample_does_not_hide_physical_crossing(self):
    detector = LateralEventDetector(cooldown=0.0)
    common = {
      "steering_rate_deg": -150.0,
      "desired_lateral_accel": 0.5,
    ...
    assert event is not None
    assert event.event_type == "centerOvershoot"
    _assert_approx_equal(self, event.occurred_mono_time, 0.15)

  def test_raw_torque_spike_is_possible_interaction_not_confirmed_press(self):
  ...
  def test_sustained_steering_pressed_is_confirmed_and_does_not_suppress(self):
    event = stall_release_event(
      LateralEventDetector(cooldown=0.0),
      stationary_kwargs=[
  ...
  def test_trigger_road_bump_is_distinct_from_transient_window_bump(self):
  ...
  def test_substantial_road_bump_remains_emitted_and_marked(self):
    event = stall_release_event(
      LateralEventDetector(cooldown=0.0),
      stationary_kwargs=[
  ...
  def test_stall_release_contains_three_distinct_actual_damping_snapshots(self):
    event = stall_release_event(
      LateralEventDetector(cooldown=0.0),
      release_kwargs=[
        {
  ...
  def test_optional_vehicle_damping_fields_can_be_absent(self):
    event = stall_release_event(LateralEventDetector(cooldown=0.0))
    assert event is not None and event.evidence is not None
  ...
  def test_non_hyundai_does_not_reuse_legacy_torque_state_d_as_actual_damping(self):
    event = stall_release_event(
      LateralEventDetector(cooldown=0.0),
      release_kwargs=[
        {"damping_applied": 4.0},
...
class TestUnwindAndTurnStop(OpenpilotTestCase):
  def test_unwind_progress_deficit_detects_moving_but_lagging_wheel(self):
  ...
  def test_unwind_progress_uses_angle_dependent_duration(self, angle, minimum_duration):
  ...
  def test_driver_assist_immediately_confirms_unwind_deficit(self):
    ...
    _assert_approx_equal(self, event.evidence.driver_assist_mono_time, event.detected_mono_time)
    assert event.evidence.driver_assist_torque < 0.0
    assert event.evidence.wheel_rate_increase_after_assist_deg_s > 0.0

  def test_unwind_neutral_crossings_are_interpolated_and_trigger_torque_is_retained(self):
    event = unwind_progress_event(driver_assist=True)
    assert event is not None and event.evidence is not None
    evidence = event.evidence
    _assert_approx_equal(self, evidence.requested_torque_neutral_cross_mono_time, 0.595)
    _assert_approx_equal(self, evidence.applied_torque_neutral_cross_mono_time, 0.695)
    _assert_approx_equal(self, evidence.unwind_command_delay_s, evidence.requested_torque_neutral_cross_mono_time - evidence.unwind_episode_start_mono_time)
    assert evidence.unwind_requested_torque_at_trigger != event.evidence.driver_assist_torque

  def test_reference_recommit_cancels_apparent_unwind(self):
    detector = LateralEventDetector(cooldown=0.0)
    events = []
  ...
  def test_unwind_road_bump_is_attribution_not_suppression(self):
    event = unwind_progress_event(driver_assist=True, road_confounded=True)
    assert event is not None and event.evidence is not None
    assert event.evidence.road_confounded_any

  def test_legacy_stationary_late_unwind_does_not_duplicate_progress_event(self):
    detector = LateralEventDetector(cooldown=0.0)
    events = []
  ...
  def test_turn_stop_turn_detects_one_strong_release(self):
    ...
    assert event.evidence.analysis_window_before_s == 6.0
    assert event.evidence.analysis_window_after_s == 2.0
    _assert_approx_equal(self, event.occurred_mono_time, event.evidence.release_mono_time)
    assert event.detected_mono_time > event.occurred_mono_time
    _assert_approx_equal(self, event.evidence.turn_stop_pre_dwell_progress_deg, 18.0)
    _assert_approx_equal(self, event.evidence.turn_stop_post_dwell_progress_deg, 21.0)
    assert event.evidence.driver_causation == DRIVER_CAUSATION_AUTONOMOUS_ONLY

  def test_turn_stop_turn_detects_major_slowdown_without_sub_eight_rate(self):
    event = turn_stop_turn_event(
      movement_rate=200.0,
      dwell_rate=40.0,
      release_rate=90.0,
    ...
    assert event is not None and event.evidence is not None
    _assert_approx_equal(self, event.evidence.minimum_dwell_rate_deg_s, 40.0)
    _assert_approx_equal(self, event.evidence.rate_reduction_ratio, 0.8)
  ...
  def test_turn_stop_turn_restart_classification(self, movement_rate, release_rate, classification):
    event = turn_stop_turn_event(
  ...
  def test_turn_stop_turn_tracks_driver_phases_and_actual_damping(self):
    event = turn_stop_turn_event(
      driver_before=True,
      driver_during=False,
      driver_after=True,
    ...
    _assert_approx_equal(self, evidence.turn_stop_actual_damping_amount, 0.03)
    assert evidence.turn_stop_actual_damping_state == "damping"
    _assert_approx_equal(self, evidence.turn_stop_breakaway_latch, 0.8)
    assert evidence.turn_stop_damping_version == 2

  def test_turn_stop_turn_non_hyundai_optional_damping_is_absent(self):
    event = turn_stop_turn_event()
    assert event is not None and event.evidence is not None
  ...
  def test_turn_stop_turn_road_bump_is_retained_as_attribution(self):
    event = turn_stop_turn_event(road_confounded=True)
    assert event is not None and event.evidence is not None
    assert event.evidence.road_confounded_any

  def test_intentional_driver_hold_without_reference_demand_is_silent(self):
    event = turn_stop_turn_event(
      demand=False,
      driver_before=True,
      driver_during=True,
  ...
  def test_steady_constant_radius_hold_does_not_emit_without_release(self):
    detector = LateralEventDetector(cooldown=0.0)
    detector.update(sample(
      0.0,
  ...
  def test_constant_radius_pause_then_release_without_reference_motion_is_silent(self):
    assert turn_stop_turn_event(demand=False) is None

  def test_near_center_pause_and_tiny_corrections_are_silent(self):
  ...
  def test_short_weak_turn_stop_dwell_is_silent(self):
  ...
  def test_turn_stop_dwell_past_max_never_emits(self):
    event = turn_stop_turn_event(dwell_s=TURN_STOP_MAX_DWELL_S + 0.45)
    assert event is None

  def test_turn_stop_dwell_just_under_max_with_valid_release_emits(self):
    event = turn_stop_turn_event(dwell_s=TURN_STOP_MAX_DWELL_S - 0.05)
    assert event is not None and event.evidence is not None
    assert event.event_type == "turnStopTurn"
    _assert_approx_equal(self, event.evidence.dwell_duration_s, TURN_STOP_MAX_DWELL_S - 0.05)
...
class TestCrownOffsetUnwind(OpenpilotTestCase):
  def test_crown_neutral_timestamps_symmetric_across_mirrored_turns(self):
    ...
    _assert_approx_equal(self, left.evidence.requested_crown_neutral_mono_time, right.evidence.requested_crown_neutral_mono_time)
    _assert_approx_equal(self, left.evidence.applied_crown_neutral_mono_time, right.evidence.applied_crown_neutral_mono_time)
  ...
  def test_crown_neutral_matches_literal_zero_when_neutral_is_zero(self):
    ...
    assert event is not None and event.evidence is not None
    evidence = event.evidence
    _assert_approx_equal(self, evidence.requested_crown_neutral_mono_time, evidence.requested_torque_neutral_cross_mono_time, delta=0.015)
    _assert_approx_equal(self, evidence.applied_crown_neutral_mono_time, evidence.applied_torque_neutral_cross_mono_time, delta=0.015)
...
class TestUnwindRebound(OpenpilotTestCase):
  def test_unwind_rebound_cycles_are_tracked(self):
    event = unwind_rebound_event(rebound=True)
    assert event is not None and event.evidence is not None
    evidence = event.evidence
    _assert_approx_equal(self, evidence.unwind_rebound_start_mono_time, 0.62)
    _assert_approx_equal(self, evidence.unwind_rebound_max_magnitude, 0.50)
  ...
  def test_unwind_clean_release_records_no_rebound(self):
...
class TestUnwindReboundEdgeCases(OpenpilotTestCase):
  def test_rebound_does_not_arm_without_a_genuine_prior_hold(self):
  ...
  def test_crown_rebound_bookkeeping_advances_on_a_sample_claimed_by_handoff(self):
    ...
    _assert_approx_equal(self, episode.previous_mono_time, 10.01)
    _assert_approx_equal(self, episode.rebound_duration_s, 0.01)
  ...
  def test_rescue_shortly_after_deficit_start_is_still_intervention_backed(self):
  ...
  def test_road_confounded_assist_deficit_still_emits_at_full_confidence(self):
    event = unwind_progress_event(driver_assist=True, road_confounded=True)
    assert event is not None and event.evidence is not None
    assert event.event_type == "unwindProgressDeficit"
    _assert_approx_equal(self, event.confidence, 0.95)
  ...
  def test_road_confounded_non_assist_deficit_records_metrics_without_penalty(self):
    ...
    assert event is not None and event.evidence is not None
    _assert_approx_equal(self, event.confidence, 0.90)
    assert event.evidence.road_confounded_fraction > 0.0
    assert event.evidence.max_vertical_accel_deviation > 0.0

  # --- v6: episode identity, re-arm guard, lifetime cap (section 3, A2) -----

  def test_turn_stop_event_keeps_snapshot_key_after_wheel_recenters(self):
    ...
    assert event is None
    _assert_approx_equal(self, detector.episode_start, 0.0)
    ...
    assert event is not None and event.evidence is not None
    assert event.event_type == "turnStopTurn"
    _assert_approx_equal(self, event.evidence.episode_start_mono_time, 0.0)
    assert event.evidence.episode_key == "lat:0"
    assert detector.episode_start is None  # confirms the live state had moved on

  def test_episode_rearm_guard_blocks_tracking_active_alone_near_center(self):
    detector = LateralEventDetector(cooldown=0.0)
    ...
    _assert_approx_equal(self, detector.episode_start, now)

  def test_episode_lifetime_cap_forces_reset_after_max_lifetime(self):
    detector = LateralEventDetector(cooldown=0.0)
    ...
    _assert_approx_equal(self, first_start, 0.0)
    ...
    for index in range(1, ticks_to_boundary + 1):
      ...
      _assert_approx_equal(self, detector.episode_start, first_start)

    now = round(now + 0.1, 1)
    assert now > EPISODE_MAX_LIFETIME_S
    detector.update(sample(now, steering_angle_deg=30.0, steering_rate_deg=0.0, reference_rate=0.0))
    _assert_approx_equal(self, detector.episode_start, now)
    _assert_not_approx_equal(self, detector.episode_start, first_start)

  # --- v6: route discontinuity reset (section 2 rule 5, amendment A8) -------

  def test_backwards_discontinuity_resets_cooldown_and_history_with_default_cooldown(self):
    ...
    _assert_approx_equal(self, event.occurred_mono_time, 700.01)
  ...
  def test_road_evidence_window_clamped_to_trimmed_history(self):
    ...
    with mock.patch.object(lat_event_detector, "EVIDENCE_HISTORY_S", 1.0):
      event = unwind_progress_event()
    assert event is not None and event.evidence is not None
    evidence = event.evidence
    _assert_approx_equal(self, evidence.episode_start_mono_time, 0.0)
    ...
    _assert_not_approx_equal(self, evidence.road_evidence_window_start_mono_time, naive_window_start)
    _assert_approx_equal(self, evidence.road_evidence_window_start_mono_time, 1.52)
