"""Pure composition core for the modular BLaTv2 numerical artifacts.

This module adds no control law. It composes, at one explicit physical effect
time, the existing intent/reference, rack mapping, vehicle profile, plant,
observer, and computed-torque APIs. The sole unconstrained request remains
``plant.compute_inverse_torque``. No rate limit, clamp, actuation, integral,
filter, maneuver state, or additional speed schedule exists here.
...
class ModularControllerCore:
  ...
  def __init__(
    ...
    *,
    ...
    self._reference_times = [0.0]
    self._reference_curvatures = [0.0]
    self._reference_curvature_rates = [0.0]
    self._reference_curvature_accelerations = [0.0]
    self._reference_speeds = [0.0]
    self._reference_speed_rates = [0.0]
    self._reference_speed_accelerations = [0.0]
    ...
    self._rack_angles = [0.0]
    self._rack_rates = [0.0]
    self._rack_accelerations = [0.0]
  ...
  def prime_applied_history(self, current_applied_torque: float) -> None:
    ...
    An engagement starts without controller-frame history, while the physical
    rack already carries a command through its transport delay. Repeating the
    exact measured current applied torque across the fixed history ring is the
    deterministic Markov initialization for that unknown pre-engagement
    interval. It predicts neither path nor future torque and is replaced one
    sample at a time by recorded response after engagement.
    """
    applied = float(current_applied_torque)
    if not math.isfinite(applied):
      raise ValueError("applied-history prime must be finite")
  ...
  def update(
    ...
    *,
    ...
    """Compose one frame without constraining or actuating its raw request."""
    if (
    ...
    self._query_times[0] = physical_effect_plan_s
    try:
    ...
    elif reference_status.scalar_only:
      self.result.status = CoreStatus.DEGRADED_SCALAR_ONLY
